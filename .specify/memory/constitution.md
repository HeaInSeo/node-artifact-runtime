# node-artifact-runtime Constitution
...
node-artifact-runtime does **not** treat NodeVault §4 as an independent platform
...
4. every scoped/domain/component authority required by the node-artifact-runtime
   task is explicitly present in the task `Authority Snapshot`;
...
node-artifact-runtime work that depends on platform structure/ownership/
call-direction or a specific artifact-runtime/API contract must carry the exact
CURRENT architecture and relevant scoped/component contract directly in the
...
- **race tests** (IMPLEMENTED — `make test` race variant): concurrency safety.

## §1.10 — "do not record what you did not observe"

**Authority: CURRENT platform invariant under `AR-2026-08-17.1`. Enforcement in
this repo: PROPOSED where no deterministic local gate exists.**
node-artifact-runtime has no deterministic rule that generally enforces this
invariant today. The platform invariant's authority status and this repo's local
enforcement status are separate axes.
